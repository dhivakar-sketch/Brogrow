...
const parseMetrics = (a) => { try { return typeof a?.metricsJson === 'string' ? JSON.parse(a.metricsJson || '{}') : (a?.metricsJson || a?.metrics || {}) } catch { return {} } }
...
function MetricBars({ metrics }) {
  if (!metrics.length) return <div className="empty-state"><p>No metric details available.</p></div>
  return <div style={{display:'grid',gap:13}}>{metrics.map(({key,value}) => { const pct = Math.max(0, Math.min(100, value)); return <div key={key}><div style={{display:'flex',justifyContent:'space-between',gap:12,marginBottom:6}}><span>{pretty(key)}</span><strong>{value.toFixed(1)}</strong></div><div style={{height:8,borderRadius:99,background:'rgba(255,255,255,.08)',overflow:'hidden'}}><div style={{height:'100%',width:`${pct}%`,background:'linear-gradient(90deg,#6366f1,#06b6d4)',borderRadius:99}} /></div></div> })}</div>
}
...
export default function PlayerProfileReport({ profile = {}, assessments = [], insights = [], athleteName = 'Athlete' }) {
  ...
  const averageMetric = metrics.length ? metrics.reduce((sum,x) => sum + x.value, 0) / metrics.length : 0

  return <div className="panel player-profile-report">
    <div className="panel-header split-row"><div><span className="eyebrow">Athlete Report</span><h2 style={{marginTop:4}}>{profile.athleteName || athleteName}</h2><span className="muted">{latest?.sport || profile.primarySport || 'Sport'} · Complete performance profile</span></div><span className={`coach-review-status ${verified ? 'approved' : ''}`}>{verified ? '✓ Coach Verified' : '⏳ Pending Verification'}</span></div>

    <div style={{display:'grid',gridTemplateColumns:'1.25fr .75fr',gap:14,marginBottom:16}}>
      <div className="insight-box"><span className="muted">Athlete overview</span><div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginTop:14}}>
        <div><small className="muted">Primary sport</small><strong style={{display:'block',marginTop:5}}>{profile.primarySport || latest?.sport || '—'}</strong></div><div><small className="muted">Skill level</small><strong style={{display:'block',marginTop:5}}>{profile.skillLevel || '—'}</strong></div><div><small className="muted">Training</small><strong style={{display:'block',marginTop:5}}>{profile.yearsOfTraining ? `${profile.yearsOfTraining} years` : '—'}</strong></div><div><small className="muted">Position</small><strong style={{display:'block',marginTop:5}}>{profile.playingPosition || '—'}</strong></div><div><small className="muted">Academy</small><strong style={{display:'block',marginTop:5}}>{profile.academyName || '—'}</strong></div><div><small className="muted">Coach</small><strong style={{display:'block',marginTop:5}}>{profile.coachName || '—'}</strong></div>
      </div></div>
    ...
    {latest ? <>
      <div className="panel" style={{margin:'0 0 16px'}}><div className="panel-header"><div><h2>📊 Performance metrics</h2><span className="muted">Latest assessment · {metrics.length} tracked metrics</span></div><strong>{averageMetric.toFixed(1)} avg.</strong></div><MetricBars metrics={metrics} /></div>
      <div className="two-column" style={{marginBottom:16}}><div className="panel" style={{margin:0}}><div className="panel-header"><h2>💪 Strengths</h2></div>{strengths.map(x => <div key={x.key} style={{display:'flex',justifyContent:'space-between',padding:'11px 0',borderBottom:'1px solid rgba(255,255,255,.07)'}}><span>{pretty(x.key)}</span><strong>{x.value}</strong></div>)}</div><div className="panel" style={{margin:0}}><div className="panel-header"><h2>🎯 Development focus</h2></div>{focus.map(x => <div key={x.key} style={{display:'flex',justifyContent:'space-between',padding:'11px 0',borderBottom:'1px solid rgba(255,255,255,.07)'}}><span>{pretty(x.key)}</span><strong>{x.value}</strong></div>)}</div></div>
